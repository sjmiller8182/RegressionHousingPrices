...
Partials <- subset(df_ThreeNeighborhoods[which(df_ThreeNeighborhoods$SaleCondition == "Partial"),])

summary(lm(Partials$SalePrice ~ Partials$GrLivArea, data = Partials))
plot(lm(Partials$SalePrice ~ Partials$GrLivArea, data = Partials))
```
...
```{r, echo=T}
my.pred <- predict(loglogModelInteract.lm, data.frame(NAmesborhood == "BrkSide", logFeet = log(1077)))
