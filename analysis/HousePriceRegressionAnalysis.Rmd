...
We start with the logarithm of sale price as the response of living room area and build up a model with neighborhood.
The equations (1-3) below show the models considered. 
The Edwards neightborhood is used for reference.

**Base Model**

\begin{equation}
\mu \lbrace log(SalePrice) \rbrace = \hat{\beta_0} + \hat{\beta_1} (LivingRoomArea) (\#eq:Base)
\end{equation}

**Additive Model**

\begin{equation}
\mu \lbrace log(SalePrice) \rbrace = \hat{\beta_0} + \hat{\beta_1} (LivingRoomArea) +  \hat{\beta_2} (Brookside) + \hat{\beta_3} (NorthwestAmes) (\#eq:Additive)
\end{equation}
...
\hat{\beta_3} (Brookside)(LivingRoomArea) + \hat{\beta_4} (NorthwestAmes)(LivingRoomArea) (\#eq:Interaction)
\end{align}

\newpage

### ESS Tests Between Models

```{r, ESS-setup, results='hide', echo=FALSE}
...
model.interaction.formula = log(SalePrice) ~ (GrLivArea) + 
     Neighborhood_BrkSide + 
     Neighborhood_NAmes +
     (GrLivArea) * Neighborhood_BrkSide + 
     (GrLivArea) * Neighborhood_NAmes
# additive model formula
model.add.formula = log(SalePrice) ~ (GrLivArea) + 
     Neighborhood_BrkSide + 
     Neighborhood_NAmes 
# base model formula
model.base.formula = log(SalePrice) ~ (GrLivArea)
```

**Comparison: Base Model to Additive Model**

The following ESS test provides convincing evidence that the addition of the additive neighborhood terms
is an improvement over the base model (p-value < 0.0001).

```{r, ESS1, echo=FALSE}
# fit models
model <- lm(formula = model.add.formula, data = train.mod)
model.reduced <- lm(formula = model.base.formula, data = train.mod)
# ESS test on models
...
```{r, ESS2, echo=FALSE}
# fit models
model <- lm(formula = model.interaction.formula, data = train.mod)
model.reduced <- lm(formula = model.add.formula, data = train.mod)
# ESS test on models
anova(model.reduced, model)
```

Based on the two ESS tests, the interaction model appears to be significant;
thus, we will continue with the interaction model.
...
The three models were trained and validated on the training dataset using 10-fold cross validation. The table below summerizes the performance of the models with RMSE, adjusted $R^2$, and PRESS. These results show that the interaction model produces the best performance, which is consistent with the result of the ESS test.
...
# Train the model
model.cv <- train(model.interaction.formula, 
                    data = train.mod,
                    method = 'lm',
...
train.control <- trainControl(method = "cv", number = 10)
# Train the model
model.reduced.cv <- train(model.add.formula, 
                    data = train.mod,
                    method = 'lm',
                    trControl = train.control)
# print model summary
...
train.control <- trainControl(method = "cv", number = 10)
# Train the model
model.base.cv <- train(model.base.formula, 
                    data = train.mod,
                    method = 'lm',
                    trControl = train.control)
# print model summary
...
# get the CV results
base.red <- model.base.cv$results

# get cross-validated PRESS statistic
PCV.base <- PRESS.cv(model.base.cv)
```

```{r, echo=FALSE}
# print accuracy metrics to md table
kable(data.frame('Model' = c('Interaction Model', 'Additive Model', 'Base Model'), 
                 'RMSE'=c(res$RMSE, res.red$RMSE, base.red$RMSE),
                 'CV Press'=c(PCV, PCV.red, PCV.base),
                 'Adjused R Squared'=c(res$Rsquared, res.red$Rsquared, base.red$Rsquared)),
      "latex", booktabs = T)  %>%
...
# Suggested output from Forward Selection:
...
# Suggested output from Backward Elimination:
...
# Suggested output from Stepwise Regression:
